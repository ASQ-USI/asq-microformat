...
(function(){
  ...
  * @param {string} html The html string to parse. Useful only for server-side processing.
  * @param {object} options 
  * @returns {object} tha contains questions, stats and html. if any errors occured, the return value is an an object with an 'errors' array
  */
  this.parse = function(html, options){
    ...
    $exercises.each((function(index, el){
      var exercise = this.parseExercise(index, el)
    ...
    var errors = this.errors.length === 0 // Set errors to null if there are
      ? null                              // no errors.
      : this.errors;
