...
var MarkupGenerator = module.exports = function(dustInstance){
  ...
  var tmpDustRender = this.dust.render;
  this.dust.render = function(template, data, cb) {
    console.log('DUST RENDERING ', template, data);
    return tmpDustRender(template, data, cb);
  }
...
(function(){
  ...
  this.renderWelcome = function($welcomeEl, data){
    var deferred = when.defer();
     // TODO: AWFUL including each time! Find solution
    //require('../dusts/compiled/templates')();
    this.dust.render(this.defaultOptions.templates['welcome-screen'][this.options.userType], data, function(err, out) {
      if(err) {
        process.nextTick(function(){
          deferred.reject(err);
        });
  ...
  this.renderStats = function(question){
    var selector = '.stats[data-target-assessment="' +question.htmlId+ '"]'
    return when.map($(selector), function(stat){
  ...
  this.renderQuestions = function(questions){
    return when.map(questions, (function(question){
      return this.renderQuestion.call(this, question)
        .then(this.renderStats.bind(this))
     }).bind(this))
  }

  this.renderAll = function(exercises){
      var $ = this.$
        , userType = this.options.userType
        , deferreds = [];
        ...
        return when.map(exercises, function(exercise){
          return this.renderExercise(exercise)
            .then(function(){
              return this.renderQuestions.call(this, exercise.questions);
            }.bind(this));
        }.bind(this))
        .then(function(){
  ...
  this.render = function(html, exercises, options){
    var  deferred = when.defer()
    ...
    this.renderAll.call(this, exercises).then(function renderedFinished(){
